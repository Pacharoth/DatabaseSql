...
-- i. find the names of sailors who reserved a red boat.
select distinct sname from Reserves,Sailors,Boats where Reserves.sid=Sailors.sid and Reserves.bid = Boats.bid and Boats.color='red' ;

-- j.Find the names of sailors who reserved at least one boat.
select distinct s.sname from Sailors s, Reserves r, Boats b where r.sid=s.sid and r.bid=b.bid;


-- k.find the names of sailors who reserved red or green boats.
select distinct sname from Sailors join Reserves on Reserves.sid = Sailors.sid join Boats on Reserves.bid = Boats.bid where Boats.color ='red' 
union 
...
select distinct s.sname from Sailors s join Reserves r on r.sid=s.sid join Boats b on b.bid =r.bid where color ='red' and 
s.sid in 
...
select distinct b1.bid,b1.bname,b1.color from Boats b1 join Boats b2 on b1.bid !=b2.bid where b1.bname = b2.bname;
